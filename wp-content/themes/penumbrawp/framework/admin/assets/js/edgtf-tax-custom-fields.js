jQuery(document).ready( function($) {
    ...
    function edgtf_tax_media_remove(button_class) {
        $('body').on('click', button_class,function(){
            var $this = $(this);
            var parent = $this.closest('.form-field');
            var image = parent.find('.edgtf-tax-custom-media-url');
            ...
            if( image.val() !== '' && confirm( 'Are you sure you want to delete this file?' ) ) {
                var result = $.ajax({
                    ...
                    type: 'GET',
                    data: {
                        action: 'penumbra_edge_tax_del_image',
    ...
    function edgtfInitTaxColorpicker() {
        if ($('.edgtf-taxonomy-color-field').length) {
            $('.edgtf-taxonomy-color-field').wpColorPicker({
